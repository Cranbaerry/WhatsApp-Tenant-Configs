 ...
 * @since  1.6.0
 */
class WhatsapptenantsconfigController extends BaseController
{
	/**
	...
	public function publish()
	{
		...
		if ($user->authorise('core.edit', 'com_dt_whatsapp_tenants_configs') || $user->authorise('core.edit.state', 'com_dt_whatsapp_tenants_configs'))
		{
			$model = $this->getModel('Whatsapptenantsconfig', 'Site');
	...
	public function checkin()
	{
		...
		if ($user->authorise('core.manage', 'com_dt_whatsapp_tenants_configs') || $item->checked_out == $user->id) { 
			...
			if ($return === false)
			{
				...
				$this->setRedirect(Route::_('index.php?option=com_dt_whatsapp_tenants_configs&view=whatsapptenantsconfig' . '&id=' . $id, false), $message, 'error');
				return false;
			}
			else
			{
				// Checkin succeeded.
				$message = Text::_('COM_DT_WHATSAPP_TENANTS_CONFIGS_CHECKEDIN_SUCCESSFULLY');
				$this->setRedirect(Route::_('index.php?option=com_dt_whatsapp_tenants_configs&view=whatsapptenantsconfig' . '&id=' . $id, false), $message);
				return true;
			}
		}
		else
